...
async def get_auth_data(_request: Request) -> JSONResponse:
    ...
    grace_config = _request.query_params
    response = await authenticate.get_auth_data(grace_config=grace_config, headers=headers)
...
async def verify_auth_token(_request: Request) -> JSONResponse:
    ...
    grace_config = _request.query_params
    auth_provider = AuthFactory.get_auth_provider()
    response = await auth_provider.extract_and_verify_token(grace_config, headers=headers)
    return JSONResponse(response.model_dump(mode="json"))
